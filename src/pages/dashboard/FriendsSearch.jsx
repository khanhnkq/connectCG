        ...
        <div className="flex flex-1 h-full overflow-hidden relative">
            ...
            <main className="flex-1 overflow-y-auto bg-background-dark p-4 md:p-8 custom-scrollbar">
                <div className="max-w-[1600px] mx-auto">
                    {/* Header */}
                    <div className="mb-6">
                        <h1 className="text-2xl font-bold text-white">Bạn bè của tôi</h1>
                        <p className="text-text-secondary text-sm mt-1">
                            Tìm kiếm và quản lý danh sách bạn bè của bạn ({pagination.totalElements} người)
                        </p>
                    </div>
                    ...
                    {loading && friends.length === 0 && (
                        <div className="flex items-center justify-center py-20">
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <p className="text-text-secondary font-bold">Đang tải danh sách bạn bè...</p>
                            </div>
                        </div>
                    )}
                    ...
                    {!loading && friends.length === 0 && (
                        <div className="flex items-center justify-center py-20">
                            <div className="text-center">
                                <span className="material-symbols-outlined text-6xl text-text-secondary/30 mb-4">group_off</span>
                                <p className="text-text-secondary text-lg">Không tìm thấy bạn bè nào</p>
                                <p className="text-text-secondary/60 text-sm mt-2">Thử thay đổi bộ lọc tìm kiếm</p>
                            </div>
                        </div>
                    )}
                                            ...
                                            {friend.gender && (
                                                <div className="flex items-center gap-2 mb-4">
                                                    ...
                                                    <span className="text-text-secondary text-sm">
                                                        {friend.gender === 'MALE' ? 'Nam' : friend.gender === 'FEMALE' ? 'Nữ' : 'Khác'}
                                                    </span>
                                                </div>
                                            )}
            ...
            <aside className="w-full md:w-[320px] lg:w-[340px] flex flex-col border-l border-[#342418] bg-[#221710] z-10 overflow-y-auto custom-scrollbar flex-none hidden md:flex">
                <div className="p-5">
                    ...
                    <div className="mb-6">
                        <label className="text-white text-sm font-bold mb-2 block">Tìm theo tên</label>
                    ...
                    <div className="mb-6">
                        <label className="text-white text-sm font-bold mb-3 block">Giới tính</label>
                        <div className="space-y-2">
                            {[
                                ...
                                <label key={option.value} className="flex items-center gap-3 cursor-pointer group/label">
                                    <div className={`size-4 rounded-full border flex items-center justify-center ${searchFilters.gender === option.value ? 'border-primary' : 'border-[#493222]'}`}>
                                        {searchFilters.gender === option.value && <div className="size-2 rounded-full bg-primary" />}
                                    </div>
                                    <input
                                        type="radio"
                                        name="gender"
                                        ...
                                        className="hidden"
                                    />
                                    <span className={`text-sm group-hover/label:text-white transition-colors ${searchFilters.gender === option.value ? 'text-white' : 'text-text-secondary'}`}>
                                        {option.label}
                                    </span>
                                </label>
                            ))}
                    ...
                    <div className="rounded-xl border border-[#342418] bg-[#2a1d15] p-4">
                        <p className="text-text-secondary text-xs mb-2">Kết quả tìm kiếm</p>
                        <p className="text-white text-2xl font-bold">{pagination.totalElements}</p>
