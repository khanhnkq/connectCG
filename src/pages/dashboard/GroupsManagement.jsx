...
export default function GroupsManagement() {
  ...
  const fetchGroups = useCallback(
    async (isInitialLoad = false) => {
      ...
      try {
        ...
        if (debouncedSearchQuery.trim()) {
          const response = await searchGroups(debouncedSearchQuery, discoverPageRef.current);
          const newData = response.content || response;
          const isLast = response.last ?? true;

          setDiscoverGroups(prev => isInitialLoad ? newData : [...prev, ...newData]);
          ...
          switch (activeTab) {
            case "my":
              ...
              if (promises.length === 0) {
                ...
                results.forEach((res, index) => {
                  ...
                  if (type === 'managed') {
                    setManagedGroups(prev => isInitialLoad ? newData : [...prev, ...newData]);
                    mMore = !isLast;
                    setManagedHasMore(!isLast);
                    if (!isLast) managedPageRef.current += 1;
                  } else {
                    setJoinedGroups(prev => isInitialLoad ? newData : [...prev, ...newData]);
                    jMore = !isLast;
            ...
            case "discover":
              ...
              setDiscoverGroups(prev => isInitialLoad ? data : [...prev, ...data]);
        ...
        if (error.response?.status !== 400) toast.error("Không thể tải danh sách nhóm.");
      } finally {
        setLoading(false);
    ...
    [activeTab, debouncedSearchQuery, managedHasMore, joinedHasMore, discoverHasMore, hasMore]
  );
  ...
  const handleJoinGroup = async (groupId) => {
    try {
      ...
      if (group?.privacy === "PUBLIC") {
        ...
        setDiscoverGroups(prev => prev.map(g => g.id === groupId ? { ...g, currentUserStatus: "ACCEPTED" } : g));
        fetchGroups(true);
      } else {
        toast.success("Đã gửi yêu cầu gia nhập nhóm!");
        setDiscoverGroups(prev => prev.map(g => g.id === groupId ? { ...g, currentUserStatus: "REQUESTED" } : g));
      }
    } catch (error) { toast.error(error.response?.data?.message || "Không thể thực hiện yêu cầu."); }
  };
  ...
  const displayedGroups = activeTab === "my" ? [] : (activeTab === "discover" ? discoverGroups : pendingInvitations);
        ...
        <div className="sticky top-0 z-40 bg-background-main/95 backdrop-blur-xl border-b border-border-main p-3 md:p-4 flex flex-col md:flex-row justify-between items-center px-4 md:px-8 gap-3 md:gap-0">
          <div className="flex bg-surface-main p-1 rounded-2xl border border-border-main w-full md:w-auto">
            {["my", "discover", "invites"].map((tab) => (
              <button key={tab} onClick={() => handleTabChange(tab)} className={`flex-1 md:flex-none px-4 md:px-6 py-2 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all relative ${activeTab === tab ? "bg-primary text-text-main shadow-lg" : "text-text-secondary hover:text-primary"}`}>
                {tab === "my" ? "Của tôi" : tab === "discover" ? "Khám phá" : "Lời mời"}
                {tab === "invites" && pendingInvitations.length > 0 && (
                  <span className="absolute -top-1 -right-1 flex h-4 w-4">
                    <span className="animate-ping absolute h-full w-full rounded-full bg-red-400 opacity-75"></span>
                    <span className="relative rounded-full h-4 w-4 bg-red-500 text-[10px] flex items-center justify-center text-white font-bold">{pendingInvitations.length}</span>
                  </span>
                )}
              </button>
            ))}
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-80 group xl:hidden">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="block w-full pl-9 md:pl-11 pr-4 py-2 md:py-2.5 border border-border-main rounded-2xl bg-surface-main text-text-main text-[10px] md:text-xs" placeholder="Tìm kiếm..." />
            </div>
            <Link to="/dashboard/groups/create" className="flex items-center justify-center gap-2 h-9 md:h-auto px-3 md:px-6 py-2 md:py-2.5 rounded-full bg-primary text-text-main hover:bg-orange-600 font-black text-xs uppercase shadow-lg"><Plus size={18} /><span className="hidden md:inline">Tạo nhóm</span></Link>
          </div>
        </div>
        ...
        <div className="px-4 md:px-8 py-6 md:py-10">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24">
              ...
              <p className="text-text-secondary font-bold uppercase tracking-widest">Đang tải...</p>
            </div>
          ) : (
            <>
              {searchQuery.trim() !== "" && (
                <div className="mb-8"><h3 className="text-sm font-black text-primary uppercase tracking-[0.2em] mb-6 flex items-center gap-2"><Search size={18} />Tìm thấy {activeTab === "my" ? (filteredGroups(managedGroups).length + filteredGroups(joinedGroups).length) : filteredGroups(displayedGroups).length} kết quả cho "{searchQuery}"</h3></div>
              )}
                ...
                <div className="space-y-12">
                  {managedGroups.length > 0 && (
                    <div className="space-y-6">
                      <div className="flex items-center gap-4"><h2 className="text-lg font-black text-text-main uppercase tracking-widest flex items-center gap-2"><ShieldCheck className="text-orange-500" size={24} />Nhóm bạn quản lý</h2><div className="h-px flex-1 bg-gradient-to-r from-border-main to-transparent" /></div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredGroups(managedGroups).map(g => <GroupCard key={g.id} group={g} activeTab={activeTab} isAdmin={true} onNavigate={navigate} />)}
                      </div>
                    </div>
                  )}
                  {joinedGroups.length > 0 && (
                    <div className="space-y-6">
                      <div className="flex items-center gap-4"><h2 className="text-lg font-black text-text-main uppercase tracking-widest flex items-center gap-2"><Users className="text-primary" size={24} />Nhóm bạn tham gia</h2><div className="h-px flex-1 bg-gradient-to-r from-border-main to-transparent" /></div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredGroups(joinedGroups).map(g => <GroupCard key={g.id} group={g} activeTab={activeTab} isAdmin={false} onNavigate={navigate} />)}
                      </div>
                    </div>
                  )}
                </div>
                ...
                filteredGroups(displayedGroups).length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredGroups(displayedGroups).map(g => (
                      <GroupCard key={g.id} group={g} activeTab={activeTab} isAdmin={checkIfAdmin(g)} onAccept={handleAcceptInvite} onDecline={handleDeclineInvite} onJoin={handleJoinGroup} onCancelRequest={handleCancelJoinRequest} onNavigate={navigate} />
                    ))}
                  </div>
                ) : <div className="py-24 text-center opacity-50"><Users size={60} className="mx-auto mb-4" /><p>Không tìm thấy nhóm phù hợp.</p></div>
              )}

              <div ref={loaderRef} className="py-10 flex justify-center w-full">
                {isFetchingMore ? <div className="size-8 border-4 border-primary/20 border-t-primary rounded-full animate-spin" /> : (!hasMore && !loading && <span className="text-[10px] opacity-30 font-black uppercase tracking-widest">Bạn đã xem hết tất cả</span>)}
              </div>
            </>
          )}
        </div>
      </div>
      <GroupsRightSidebar searchQuery={searchQuery} setSearchQuery={setSearchQuery} activeTab={activeTab} displayedGroupsLength={activeTab === "my" ? (managedGroups.length + joinedGroups.length) : displayedGroups.length} />
    </div>
  );
}
