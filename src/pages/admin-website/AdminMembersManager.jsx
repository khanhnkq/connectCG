...
const AdminMembersManager = () => {
    ...
    const fetchUsers = async (page, keyword, role) => {
        try {
            setLoading(true);
            const data = await getAllUsers({ page, keyword, role, size: pagination.pageSize });
    ...
    const handleRoleUpdate = (userId, userName, currentRole) => {
        ...
        const roleLabel = newRole === 'ADMIN' ? 'Quản trị viên' : 'Người dùng thường';
        ...
        setConfirmConfig({
            ...
            onConfirm: async () => {
                try {
                    ...
                    const errorMsg = error.response?.data?.message || error.message || "Không thể cập nhật vai trò";
    ...
    return (
        <AdminLayout title="Quản lý người dùng" activeTab="Users" brandName="Social Admin">
            <div className="p-8 space-y-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-surface-dark/20 p-6 rounded-2xl border border-border-dark/50 shadow-xl">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-2xl font-black text-white tracking-tight">Danh sách người dùng</h2>
                    </div>
                        ...
                        <tbody className="divide-y divide-border-dark/30 text-sm">
                            {members.map((member, index) => (
                                <tr key={member.id} className="hover:bg-surface-dark/40 transition-colors text-white/90 group">
                                    ...
                                    <td className="px-6 py-5">
                                        <span className={`px-2.5 py-1 text-[9px] font-black uppercase rounded-lg border flex items-center gap-1.5 w-fit ${member.role === 'ADMIN'
                                    ...
                                    <td className="px-6 py-5 text-right space-x-2">
                                        {member.id !== currentUserId && (
                                            <button
                                                onClick={() => handleRoleUpdate(member.id, member.name, member.role)}
                                                className="p-2 hover:bg-primary/10 rounded-xl text-text-muted hover:text-primary transition-all"
                                                title={member.role === 'USER' ? "Nâng cấp lên ADMIN" : "Hạ cấp xuống USER"}
                                            >
                                                <span className="material-symbols-outlined text-lg">
                                                    {member.role === 'USER' ? 'upgrade' : 'vertical_align_bottom'}
                                                </span>
                                            </button>
                                        )}
                ...
                <div className="flex justify-between items-center bg-surface-dark/20 p-6 rounded-2xl border border-border-dark/50">
                    <div className="text-text-muted text-xs font-bold">
                        Hiển thị <span className="text-white">{members.length}</span> trên <span className="text-white">{pagination.totalElements}</span> người dùng
                    </div>
