...
export default function ReportModal({
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="relative w-[360px] rounded-[18px] bg-surface-main border border-border-main p-[22px] text-text-main shadow-[0_25px_60px_rgba(0,0,0,0.4)] font-sans">
        {/* CLOSE */}
        <button
          onClick={onClose}
          disabled={submitting}
        ...
        {/* QUESTION */}
        {question && <p className="mb-2 text-[13px]">{question}</p>}
        ...
        <div className="mt-4">
          ...
          <textarea
            ...
            className="h-[70px] w-full resize-none rounded-xl bg-background-main border border-border-main p-2.5 text-[12px] text-text-main outline-none placeholder:text-text-muted"
          />
        </div>
        ...
        <div className="mt-[18px] flex gap-2.5">
          <button
            onClick={onClose}
            disabled={submitting}
            className="flex-1 rounded-full px-3 py-2.5 text-[13px] text-text-secondary hover:bg-background-main disabled:opacity-50 transition-colors"
          >
            Hủy
          </button>
