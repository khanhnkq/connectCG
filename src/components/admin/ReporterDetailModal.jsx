...
const ReporterDetailModal = ({ userId, onClose, onBan }) => {
    ...
    return (
        <div className="fixed inset-0 z-[140] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            <div className="bg-surface-dark w-full max-w-md rounded-2xl border border-white/10 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                {loading ? (
                        ...
                        <div className="pt-10 px-6 pb-6">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <p className="text-text-muted text-sm">@{user.username || 'unknown'}</p>
                                </div>
                                <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${user.role === 'ADMIN' ? 'bg-indigo-500/20 text-indigo-400' : 'bg-white/5 text-text-muted'}`}>
                                    {user.role || 'USER'}
                                </span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-6">
                                <div className="bg-white/5 p-3 rounded-xl text-center">
                                    <p className="text-xs text-text-muted uppercase font-bold">Friends</p>
                                    <p className="text-lg font-black text-white">{user.friendsCount || 0}</p>
                                </div>
                                <div className="bg-white/5 p-3 rounded-xl text-center">
                                    <p className="text-xs text-text-muted uppercase font-bold">Posts</p>
                            ...
                            <div className="mt-6 flex gap-3">
                                <button onClick={onClose} className="flex-1 py-2.5 rounded-lg bg-white/5 hover:bg-white/10 text-white font-bold transition-all">
                                    Close
                                </button>
